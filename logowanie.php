...
<?php
        ...
        if (filter_input(INPUT_POST, "wyl")){
            $um->logout($db);
            header("location:index.php");
        }

        //kliknięto przycisk submit z name = zaloguj
        if (filter_input(INPUT_POST, "zal")) {
            $userId = $um->login($db); //sprawdź parametry logowania
            if ($userId > 0) {
                header("location:index.php");
            } else {
                $um->loginForm(); //Pokaż formularz logowania
                echo "<br /><div class='text-center'><p>Błędna nazwa użytkownika lub hasło!</p></div></section></div>";
            }
        } else {
            //pierwsze uruchomienie skryptu logowanie
            $um->loginForm();   
        }
?>
</div>
