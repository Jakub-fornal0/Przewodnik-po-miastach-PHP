...
<?php
        ...
        include_once('klasy/Rejestracja.php');
        include_once("klasy/baza.php");
        $bd = new baza("localhost", "root", "", "uzytkownicy");
        $rf = new Rejestracja(); //wyświetla formularz rejestracji
        
        if (filter_input(INPUT_POST, 'rejstr', FILTER_SANITIZE_FULL_SPECIAL_CHARS)) {
            $user = $rf->checkUser(); //sprawdza poprawność danych
            if ($user === NULL)
                echo "<br /><div class='text-center'><p>Niepoprawne dane rejestracji!</p></div></section></div>";
            else{
                echo "<br /><div class='text-center'<p>Rejestracja przebiegła pomyślnie. Możesz się zalogować.</p></div></section></div>";
                $user->saveDB($bd);
            }
        }
?>
